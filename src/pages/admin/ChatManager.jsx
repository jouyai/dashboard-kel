...
import { 
  UserCircleIcon, PaperAirplaneIcon, CheckCircleIcon, 
  ClockIcon, BuildingOffice2Icon, UserPlusIcon, 
...
export default function ChatManager() {
  ...
  useEffect(() => {
    ...
    if (activeTab === 'priority') {
      ...
      result = sessions.filter(s => s.status === 'live' && s.handled_by === currentUser.email);
    } else {
      result = sessions.filter(s => s.status === 'bot' || (s.status === 'live' && s.handled_by !== currentUser.email && s.handled_by !== null));
    }
  ...
  useEffect(() => {
    ...
    const messageChannel = supabase.channel(`chat:${selectedSession.id}`)
  ...
  const handleClaimChat = async () => {
    ...
    await supabase.from('chat_messages').insert([
      { session_id: selectedSession.id, sender: 'system', message: `Percakapan ini sekarang ditangani oleh Staff: ${currentUser.email}` }
    ]);
  ...
  const handleResolve = async () => {
    ...
    await supabase.from('chat_messages').insert([
      { session_id: selectedSession.id, sender: 'system', message: "Sesi live chat diakhiri. Kembali ke asisten virtual." }
    ]);
  ...
  return (
    <div className="h-[calc(100vh-140px)] flex gap-6">
      ...
      {/* --- SIDEBAR (GLASS) --- */}
      <div className="w-1/3 bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl overflow-hidden flex flex-col shadow-xl">
        
        {/* TAB FILTER */}
        <div className="p-2 grid grid-cols-3 gap-1 bg-white/5 border-b border-white/10">
            <button 
                onClick={() => setActiveTab('priority')}
                className={`text-xs py-2 rounded-lg font-bold transition flex flex-col items-center gap-1 ${activeTab === 'priority' ? 'bg-red-500/80 text-white shadow-lg' : 'text-gray-400 hover:bg-white/5'}`}
            >
                <div className="relative">
                   <FunnelIcon className="h-4 w-4" />
                   {/* Badge Counter */}
                   {sessions.filter(s => s.status === 'live' && !s.handled_by).length > 0 && (
                     <span className="absolute -top-1 -right-2 bg-white text-red-600 w-3 h-3 text-[8px] flex items-center justify-center rounded-full font-extrabold">
                       {sessions.filter(s => s.status === 'live' && !s.handled_by).length}
                     </span>
                   )}
                </div>
                Perlu Ditangani
            ...
            <button 
                ...
                className={`text-xs py-2 rounded-lg font-bold transition flex flex-col items-center gap-1 ${activeTab === 'mine' ? 'bg-blue-600 text-white shadow-lg' : 'text-gray-400 hover:bg-white/5'}`}
            >
            ...
            <button 
                ...
                className={`text-xs py-2 rounded-lg font-bold transition flex flex-col items-center gap-1 ${activeTab === 'all' ? 'bg-gray-600 text-white shadow-lg' : 'text-gray-400 hover:bg-white/5'}`}
            >
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-2">
            {filteredSessions.length === 0 && (
                <div className="text-center py-10 text-gray-500 text-xs opacity-60">Tidak ada chat di kategori ini</div>
            )}
            ...
            >
                <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-full relative ${session.status === 'live' ? 'bg-green-500/20 text-green-400' : 'bg-gray-500/20 text-gray-400'}`}>
                    ...
                    <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-center">
                          <p className="font-semibold text-gray-100 text-sm truncate">{session.user_name || 'Warga'}</p>
                        ...
                        <p className="text-[10px] text-gray-400 mt-1 truncate">
                           {session.status === 'live' && !session.handled_by ? (
                               <span className="text-red-400 font-bold">BUTUH RESPON!</span>
                           ) : session.handled_by ? (
                               <span className="text-blue-300">Dihandle: {session.handled_by.split('@')[0]}</span>
                           ) : (
                               <span>Bot mode</span>
                           )}
                        </p>
      ...
      {/* --- CHAT WINDOW (GLASS) --- */}
      <div className="flex-1 bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl shadow-xl flex flex-col overflow-hidden relative">
        {selectedSession ? (
          <>
            {/* HEADER CHAT */}
            <div className="p-4 bg-white/5 border-b border-white/10 flex justify-between items-center z-10">
              <div className="flex items-center gap-3">
                 ...
                 <div>
                    <h3 className="font-bold text-white text-lg">{selectedSession.user_name}</h3>
              ...
              {messages.map((msg, idx) => {
                ...
                return (
                  <div key={idx} className={`flex ${isUser ? 'justify-start' : 'justify-end'}`}>
                    <div className={`max-w-[75%] flex gap-3 ${isUser ? 'flex-row' : 'flex-row-reverse'}`}>
                      <div className="shrink-0 mt-1">
                        {isUser ? (
                          <div className="w-8 h-8 rounded-full bg-gray-600 flex items-center justify-center"><UserCircleIcon className="h-5 w-5 text-gray-300" /></div>
                        ) : (
                          <div className="w-8 h-8 rounded-full bg-blue-600 flex items-center justify-center"><BuildingOffice2Icon className="h-5 w-5 text-white" /></div>
                        )}
                      </div>
                      <div className={`p-3.5 rounded-2xl text-sm leading-relaxed shadow-sm ${
                          isUser ? 'bg-white/10 text-white rounded-tl-none border border-white/10' : 'bg-blue-600 text-white rounded-tr-none shadow-lg shadow-blue-900/20'
                      }`}>
                    ...
                    <div className="p-4 bg-white/5 border-t border-white/10">
                        <form onSubmit={sendReply} className="flex gap-3">
                            <input 
                                ...
                                placeholder={!selectedSession.handled_by ? "Ketik untuk mengambil alih chat..." : "Ketik balasan..."}
                                autoFocus
                            />
                            <button type="submit" disabled={!reply.trim()} className="bg-blue-600 hover:bg-blue-500 disabled:bg-gray-600 disabled:cursor-not-allowed text-white p-3 rounded-xl transition shadow-lg shadow-blue-500/20">
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-white/30">
            ...
            <p className="text-sm">Pilih tab "Perlu Ditangani" untuk melihat chat masuk.</p>
          </div>
        )}
      </div>
    </div>
